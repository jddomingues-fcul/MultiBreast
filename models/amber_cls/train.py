...
from components.train.train_utils import (
    estimate_val_loss_cls,
    estimate_val_metrics_cls,
...
def compute_loss(
    batch, model: AmberCLS, loss_fn: nn.Module, ctx, transforms
) -> torch.Tensor:
    ...
    with ctx:
        logits = model(img_or_projected=im, flat_logits=True, process_image=True)
    loss = loss_fn(logits, birads)
...
def main(cfg, debug: bool, resume_from: str | None, seed: int, device: str):
    ...
    ########################################################################
    # Model
    model = AmberCLS(model_configs=cfg.model, compute_device=device)
    model.to(device)
    model.compile()
    model.train()
    ...
    while True:
        ...
        if iter_num % cfg.trainer.eval_interval == 0:  # and iter_num > 0:
            val_loss = estimate_val_loss_cls(
            ...
            val_metrics = estimate_val_metrics_cls(
            ...
            print(
                f"iter {iter_num}: val loss {val_loss:.4f}, birads f1 {curr_bi_f1:.4f}"
            )
            ...
            if (val_loss < best_val_loss - min_delta) or (
                best_birads_f1 < curr_bi_f1 - min_delta
            ):
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--yaml_config",
        type=str,
        default="configs/train/amber_cls/baseline.yaml",
