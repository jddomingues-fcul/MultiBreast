...
def compute_model_performance(
    pats, pats_to_idx, predicted_birads, target_birads
) -> MetricsHolder:
    ...
    return MetricsHolder(
        ...
        findings_coverage_equality=0.0,
    )
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--eval_single_config",
        type=str,
        default="configs/eval_single/amber_cls/baseline_on_cesm.yaml",
    )
    args.add_argument(
    ...
    for seed_idx in range(len(seed_models)):
        ...
        while True:
            ...
            predicted_birads = [elem["predicted_class"] for elem in model_predictions]

            cached_predictions[seed_idx].extend_with_predictions(
                modality_pred=[-1] * len(predicted_birads),
                birads_pred=predicted_birads,
                findings_pred=["NOT PREDICTED"],
            )
